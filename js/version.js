...
const APP_VERSION = {
    ...
    changelog: [
 'v1.3.86 - 新增 Supabase 云端同步功能：支持 iPad 和 Mac 设备之间的数据同步（题库、错题、任务清单等）；添加云端同步、上传、下载按钮；智能合并策略：下载云端数据与本地数据合并后上传，确保两端一致；单用户模式，无需注册登录；支持在页面上直接配置 Supabase（URL 和 anonKey 保存在 localStorage）',

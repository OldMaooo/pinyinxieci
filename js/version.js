...
const APP_VERSION = {
    ...
    changelog: [
        'v1.3.108 - 锁定题库功能：强制从服务器加载题库并覆盖本地数据，禁止用户修改、添加或删除题库中的字，确保多设备题库数据一致性',
        'v1.3.107 - 修复手写识别调用错误：修正了对 Handwriting 模块属性的错误访问（strokes -> paths）和方法调用（getStrokes -> getSnapshot），确保识别功能正常工作',
        ...
        'v1.3.98 - 修复导出数据逻辑：在 exportSyncDataByRange 中明确导出过滤后的 wordBank，确保导出的文件包含题库数据，解决跨设备导入时因 ID 不匹配导致无法预览的问题；优化导入预览界面：当匹配失败时显示友好的提示信息',
        'v1.3.97 - 修复 iPad 导入预览空白问题：优先使用导入文件中的 wordBank 构建预览，解决新设备本地题库为空导致无法匹配的问题；修复 main.js 语法错误（重复变量声明）',
        'v1.3.96 - iPad 导入调试：增加详细的 Alert 弹窗提示，显示文件读取错误、解析错误和文件信息，用于排查 iPad 无法预览的问题',
        'v1.3.95 - 修复iPad导入问题：增加FileReader回退机制解决文件读取失败；优化导入预览：显示已掌握/错题/未练习/总计的详细统计；练习范围记忆折叠状态；增强Supabase同步日志以便排查网络问题',
        'v1.3.94 - 导航栏优化：删除「调试」和「诊断」按钮，将「主题」按钮改为设置下拉菜单，包含深浅色切换和鼠标离开变暗开关；鼠标离开变暗功能改为可开关控制，设置保存在localStorage中',
        'v1.3.92 - 简化空画布与下一题逻辑：去掉与跳过设置allowSkip的耦合，统一由handleEmptySubmission和showNextQuestion管理错误与重做流程，避免多次点击下一题或不会后状态锁死；保留跳过开关仅作为入口拦截，不再干扰内部状态机',
        'v1.3.91 - 新增跳过设置功能：在练习页面header添加下拉菜单，支持「可以跳过」和「不可以跳过」两种模式，不可以跳过时必须答对当前题目才能进入下一题；新增鼠标离开遮罩功能：鼠标离开页面时显示90%透明度的黑色遮罩，避免在上班时调试被同事看到',
        'v1.3.90 - 修复内存泄漏：限制history数组最大长度为100，防止15题后无法点击下一题；修复意外退出后错题未记录到按轮视图：增强savePartialIfActive的错误处理和日志；修复任务进度恢复问题：正确计算总正确数和错题列表，确保重新进入任务时恢复正确进度；实现复习任务创建：练习结束后自动创建复习任务，基于第一次练习的错题，避免重复生成',
        'v1.3.89 - 添加强制恢复三年级上册内置题库功能：下载和同步后自动强制恢复，添加「恢复题库」按钮手动强制恢复，确保iPad上被修改的题库能正确恢复',
        'v1.3.88 - 修复supabase-sync.js语法错误：删除重复的conflictResult声明；增强模块加载检查，优先检查SupabaseSync是否已加载',
        'v1.3.87 - 修复题库同步问题：在exportSyncData中添加wordBank，确保所有设备题库一致；修复导入覆盖模式：确保覆盖时真正替换所有数据；改进导入预览弹窗：将覆盖/合并选项移到弹窗底部；实现同步冲突检测：比较本地和云端数据差异，显示冲突提示和详情弹窗；修复SupabaseSync未定义错误：添加模块加载检查',
        'v1.3.86 - 新增 Supabase 云端同步功能：支持 iPad 和 Mac 设备之间的数据同步（题库、错题、任务清单等）；添加云端同步、上传、下载按钮；智能合并策略：下载云端数据与本地数据合并后上传，确保两端一致；单用户模式，无需注册登录；支持在页面上直接配置 Supabase（URL 和 anonKey 保存在 localStorage）；修复下载合并时 wordBank.forEach 错误，添加数组类型检查',
